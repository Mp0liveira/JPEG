...
void Bitstream::flushRawToFile(const std::string& filename)
{
	...
	// Escreve os bytes completos já prontos
	file.write(reinterpret_cast<char*>(data.data()), data.size());

	// Se ainda sobrou bits no buffer, escreve o último byte preenchido parcialmente
	if (num_buf8 > 0)
	{
		uint8_t last_byte = buf8 << (8 - num_buf8);  // Alinha os bits à esquerda
		file.write(reinterpret_cast<char*>(&last_byte), sizeof(last_byte));
	}
